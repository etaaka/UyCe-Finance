...
const CompanyCrud = () => {
  ...
  function addNewCompanyInformations(company: Company, financial: Financial, financialPrev: Financial, extracted : any) {
    CompanyService.add(company).then(
      async r => {
        ...
        financial.companyId = r.$id;
        await FinancialService.add(financial);
        financialPrev.companyId = r.$id;
        await FinancialService.add(financialPrev);
  ...
  function addNewFinancials(savedCompany: Company, financial: Financial, financialPrev: Financial) {
    FinancialService.listByCompanyId(savedCompany.$id!).then(
      financials => {
        const newFinancialMap: Map<number, any> = new Map();
        newFinancialMap.set(financial.year, financial)
        newFinancialMap.set(financialPrev.year, financialPrev)
        for (const f of financials.documents as any) {
  ...
  const onSelect = async (e: FileUploadSelectEvent) => {
    ...
    if (e.files && e.files[0]) {
      ...
      reader.onload = async (e) => {
        ...
        let company: any = CompanyService.extractCompanyInfo(extracted.lines);
        ...
        CompanyService.listByTaxNo(company.taxNo, company.userId).then(
          (savedCompany) => {
            ...
            if (savedCompany.documents.length > 0) {
              addNewFinancials(savedCompany.documents[0] as any, financial, financialPrev);
            } else {
              addNewCompanyInformations(company, financial, financialPrev, extracted);
            }
          }
        );
      };
